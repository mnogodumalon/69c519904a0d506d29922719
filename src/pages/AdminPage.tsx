...
import { StandorteAbteilungenViewDialog } from '@/components/dialogs/StandorteAbteilungenViewDialog';
...
const STANDORTEABTEILUNGEN_FIELDS = [
  { key: 'standort_name', label: 'Standortname', type: 'string/text' },
  { key: 'abteilung_name', label: 'Abteilungsname', type: 'string/text' },
  { key: 'standort_strasse', label: 'Straße', type: 'string/text' },
  { key: 'standort_hausnummer', label: 'Hausnummer', type: 'string/text' },
  { key: 'standort_plz', label: 'Postleitzahl', type: 'string/text' },
  { key: 'standort_ort', label: 'Ort', type: 'string/text' },
...
const MITARBEITER_FIELDS = [
  ...
  { key: 'wochenstunden', label: 'Wochenstunden', type: 'number' },
  { key: 'abteilung_ref', label: 'Abteilung', type: 'applookup/select', targetEntity: 'standorte_&_abteilungen', targetAppId: 'STANDORTE_ABTEILUNGEN', displayField: 'standort_name' },
  { key: 'mitarbeiter_notizen', label: 'Notizen', type: 'string/textarea' },
];
const SCHICHTVORLAGEN_FIELDS = [
  { key: 'schicht_name', label: 'Schichtname', type: 'string/text' },
  { key: 'schicht_kuerzel', label: 'Kürzel', type: 'string/text' },
  { key: 'schicht_beginn', label: 'Schichtbeginn (Uhrzeit)', type: 'string/text' },
  { key: 'schicht_ende', label: 'Schichtende (Uhrzeit)', type: 'string/text' },
  { key: 'pausendauer', label: 'Pausendauer (Minuten)', type: 'number' },
  { key: 'schicht_kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'fruehschicht', label: 'Frühschicht' }, { key: 'spaetschicht', label: 'Spätschicht' }, { key: 'nachtschicht', label: 'Nachtschicht' }, { key: 'tagschicht', label: 'Tagschicht' }, { key: 'sonderschicht', label: 'Sonderschicht' }] },
  { key: 'schicht_abteilung_ref', label: 'Abteilung', type: 'applookup/select', targetEntity: 'standorte_&_abteilungen', targetAppId: 'STANDORTE_ABTEILUNGEN', displayField: 'standort_name' },
  { key: 'schicht_beschreibung', label: 'Beschreibung', type: 'string/textarea' },
];
const SCHICHTPLANUNG_FIELDS = [
  { key: 'schicht_datum', label: 'Datum', type: 'date/date' },
  { key: 'mitarbeiter_ref', label: 'Mitarbeiter', type: 'applookup/select', targetEntity: 'mitarbeiter', targetAppId: 'MITARBEITER', displayField: 'vorname' },
  { key: 'schicht_ref', label: 'Schicht', type: 'applookup/select', targetEntity: 'schichtvorlagen', targetAppId: 'SCHICHTVORLAGEN', displayField: 'schicht_name' },
  { key: 'planung_abteilung_ref', label: 'Abteilung / Standort', type: 'applookup/select', targetEntity: 'standorte_&_abteilungen', targetAppId: 'STANDORTE_ABTEILUNGEN', displayField: 'standort_name' },
  { key: 'schicht_status', label: 'Status', type: 'lookup/radio', options: [{ key: 'geplant', label: 'Geplant' }, { key: 'bestaetigt', label: 'Bestätigt' }, { key: 'abwesend', label: 'Abwesend' }, { key: 'storniert', label: 'Storniert' }] },
  { key: 'planung_notizen', label: 'Bemerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'standorte_&_abteilungen', label: 'Standorte & Abteilungen', pascal: 'StandorteAbteilungen' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('standorte_&_abteilungen');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'standorte_&_abteilungen': new Set(),
    'mitarbeiter': new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'standorte_&_abteilungen': return (data as any).standorteAbteilungen as StandorteAbteilungen[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'mitarbeiter':
        lists.standorteAbteilungenList = (data as any).standorteAbteilungen ?? [];
        break;
      case 'schichtvorlagen':
        lists.standorteAbteilungenList = (data as any).standorteAbteilungen ?? [];
        break;
      case 'schichtplanung':
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'mitarbeiter' && fieldKey === 'abteilung_ref') {
      const match = (lists.standorteAbteilungenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.standort_name ?? '—';
    }
    if (entity === 'schichtvorlagen' && fieldKey === 'schicht_abteilung_ref') {
      const match = (lists.standorteAbteilungenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.standort_name ?? '—';
    }
    if (entity === 'schichtplanung' && fieldKey === 'mitarbeiter_ref') {
      const match = (lists.mitarbeiterList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'schichtplanung' && fieldKey === 'schicht_ref') {
      const match = (lists.schichtvorlagenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.schicht_name ?? '—';
    }
    if (entity === 'schichtplanung' && fieldKey === 'planung_abteilung_ref') {
      const match = (lists.standorteAbteilungenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.standort_name ?? '—';
    }
    return String(url);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'standorte_&_abteilungen': return STANDORTEABTEILUNGEN_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'standorte_&_abteilungen': return {
        create: (fields: any) => LivingAppsService.createStandorteAbteilungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateStandorteAbteilungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteStandorteAbteilungenEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'standorte_&_abteilungen' || dialogState?.entity === 'standorte_&_abteilungen') && (
        <StandorteAbteilungenDialog
          open={createEntity === 'standorte_&_abteilungen' || dialogState?.entity === 'standorte_&_abteilungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'standorte_&_abteilungen' ? handleUpdate : (fields: any) => handleCreate('standorte_&_abteilungen', fields)}
          defaultValues={dialogState?.entity === 'standorte_&_abteilungen' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'mitarbeiter' || dialogState?.entity === 'mitarbeiter') && (
        <MitarbeiterDialog
          ...
          standorteAbteilungenList={(data as any).standorteAbteilungen ?? []}
      ...
      {(createEntity === 'schichtplanung' || dialogState?.entity === 'schichtplanung') && (
        <SchichtplanungDialog
          ...
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichtplanung' ? handleUpdate : (fields: any) => handleCreate('schichtplanung', fields)}
          defaultValues={dialogState?.entity === 'schichtplanung' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'standorte_&_abteilungen' && (
        <StandorteAbteilungenViewDialog
          open={viewState?.entity === 'standorte_&_abteilungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'standorte_&_abteilungen', record: r }); }}
        />
      )}
      ...
      {viewState?.entity === 'schichtplanung' && (
        <SchichtplanungViewDialog
          open={viewState?.entity === 'schichtplanung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
