...
const MITARBEITER_FIELDS = [
  ...
  { key: 'abteilung_ref', label: 'Abteilung', type: 'applookup/select', targetEntity: 'standorte_abteilungen', targetAppId: 'STANDORTE_ABTEILUNGEN', displayField: 'standort_name' },
...
const SCHICHTVORLAGEN_FIELDS = [
  ...
  { key: 'schicht_abteilung_ref', label: 'Abteilung', type: 'applookup/select', targetEntity: 'standorte_abteilungen', targetAppId: 'STANDORTE_ABTEILUNGEN', displayField: 'standort_name' },
...
const SCHICHTPLANUNG_FIELDS = [
  ...
  { key: 'planung_abteilung_ref', label: 'Abteilung / Standort', type: 'applookup/select', targetEntity: 'standorte_abteilungen', targetAppId: 'STANDORTE_ABTEILUNGEN', displayField: 'standort_name' },
...
const ENTITY_TABS = [
  ...
  { key: 'standorte_abteilungen', label: 'Standorte & Abteilungen', pascal: 'StandorteAbteilungen' },
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'standorte_abteilungen': return (data as any).standorteAbteilungen as StandorteAbteilungen[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'mitarbeiter':
        lists.standorte_abteilungenList = (data as any).standorteAbteilungen ?? [];
        break;
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'mitarbeiter' && fieldKey === 'abteilung_ref') {
      const match = (lists.standorte_abteilungenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'standorte_abteilungen': return STANDORTEABTEILUNGEN_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'standorte_abteilungen': return {
    ...
    >
      ...
      {(createEntity === 'mitarbeiter' || dialogState?.entity === 'mitarbeiter') && (
        <MitarbeiterDialog
          ...
          standorte_abteilungenList={(data as any).standorteAbteilungen ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Mitarbeiter']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Mitarbeiter']}
        />
      )}
      {(createEntity === 'standorte_abteilungen' || dialogState?.entity === 'standorte_abteilungen') && (
        <StandorteAbteilungenDialog
          open={createEntity === 'standorte_abteilungen' || dialogState?.entity === 'standorte_abteilungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'standorte_abteilungen' ? handleUpdate : (fields: any) => handleCreate('standorte_abteilungen', fields)}
          defaultValues={dialogState?.entity === 'standorte_abteilungen' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'standorte_abteilungen' && (
        <StandorteAbteilungenViewDialog
          open={viewState?.entity === 'standorte_abteilungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'standorte_abteilungen', record: r }); }}
        />
      )}
