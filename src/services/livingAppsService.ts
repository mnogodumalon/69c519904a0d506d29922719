...
import type { StandorteAbteilungen, Mitarbeiter, Schichtvorlagen, Schichtplanung, CreateStandorteAbteilungen, CreateMitarbeiter, CreateSchichtvorlagen, CreateSchichtplanung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getStandorteAbteilungen(): Promise<StandorteAbteilungen[]> {
    ...
    return enrichLookupFields(records, 'standorte_&_abteilungen');
  }
  static async getStandorteAbteilungenEntry(id: string): Promise<StandorteAbteilungen | undefined> {
    ...
    return enrichLookupFields([record], 'standorte_&_abteilungen')[0];
  }
  static async createStandorteAbteilungenEntry(fields: CreateStandorteAbteilungen) {
    return callApi('POST', `/apps/${APP_IDS.STANDORTE_ABTEILUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'standorte_&_abteilungen') });
  }
  static async updateStandorteAbteilungenEntry(id: string, fields: Partial<CreateStandorteAbteilungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.STANDORTE_ABTEILUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'standorte_&_abteilungen') });
  }
